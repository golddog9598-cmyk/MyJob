  ...
  <div v-else class="workspace-shell">
    ...
    <div class="workspace-body">
      <header class="app-topbar">
        ...
        <div class="runtime-summary"><span :class="{ active: browserStatus.browser_running }"><Icon icon="mdi:web" />{{ browserStatus.browser_running ? 'BOSS 浏览器运行中' : 'BOSS 浏览器未启动' }}</span><span class="desktop-only">今日投递 {{ summary?.stats?.today_applications || 0 }} / {{ summary?.stats?.daily_limit || 0 }}</span></div>
        <div class="browser-actions desktop-only">
          <button :disabled="systemBusy" @click="startBrowser"><Icon icon="mdi:play-outline" />启动浏览器</button>
          <button :disabled="systemBusy || !browserStatus.browser_running" @click="checkBossLogin(false)"><Icon icon="mdi:account-check-outline" />检查登录</button>
          <button :disabled="systemBusy || !browserStatus.browser_running" @click="logoutBoss"><Icon icon="mdi:logout-variant" />登出 BOSS</button>
      ...
      <div class="mobile-runtime-actions">
        <button @click="startBrowser">启动</button><button :disabled="!browserStatus.browser_running" @click="checkBossLogin(false)">检查登录</button><button :disabled="!browserStatus.browser_running" @click="logoutBoss">登出 BOSS</button><button :disabled="!browserStatus.browser_running" @click="stopBrowser">停止</button>
      </div>
      ...
      <main id="main-content" class="workspace-main">
        <KeepAlive>
          ...
          <JobCenterView v-else-if="currentView === 'jobs'" @notify="notify" @changed="onDataChanged" />
...
async function loadSummary(force = false) {
  ...
  try { summary.value = await api.get('/api/dashboard/summary', { ttl: 10000, force }) }
...
async function startBrowser() {
  ...
  try {
    const result = await api.post('/api/system/start')
    if (result.status === 'error') throw new Error(result.message)
    await sleep(700)
    await loadSummary(true)
    await checkBossLogin(true)
  } catch (exc) { showModal('error', '浏览器启动失败', exc.message) }
  finally { systemBusy.value = false }
}

async function checkBossLogin(automatic = false) {
  if (systemBusy.value && !automatic) return
  if (!automatic) systemBusy.value = true
  try {
    const result = await api.post('/api/system/check-login')
    showModal(result.logged_in ? 'success' : 'info', result.logged_in ? '登录成功' : '请登录', result.logged_in ? 'BOSS 登录状态验证通过。' : '请登录')
    await loadSummary(true)
  } catch (exc) { showModal('error', '检查失败', exc.message) }
  finally { if (!automatic) systemBusy.value = false }
}

async function logoutBoss() {
  systemBusy.value = true
  try {
    const result = await api.post('/api/system/logout')
    showModal(result.status === 'ok' ? 'success' : 'error', result.status === 'ok' ? '已登出 BOSS' : '登出失败', result.message)
    await loadSummary(true)
...
async function stopBrowser() {
  ...
  try {
    ...
    await loadSummary(true)
    showModal('success', '已停止', 'BOSS 浏览器与消息监控已停止。')
